...
#include "model/repository/cells/Partition_mt.h"
...
NcursesView::NcursesView(std::shared_ptr<game::mt::service::events::EventBus> bus)
    : bus_(std::move(bus)) {
    initscr();
    cbreak();
    noecho();
    ...
    if (bus_) {
        bus_->subscribe<game::mt::service::events::ShotMissedEvent>(
            [this](const std::shared_ptr<game::mt::service::events::ShotMissedEvent>&) {
                last_message_ = "Shot missed";
            });
        bus_->subscribe<game::mt::service::events::ShotMissedAtCellEvent>(
            [this](const std::shared_ptr<game::mt::service::events::ShotMissedAtCellEvent>&) {
                last_message_ = "Shot missed (cell)";
            });
        bus_->subscribe<game::mt::service::events::DamageEvent>(
            [this](const std::shared_ptr<game::mt::service::events::DamageEvent>& ev) {
                last_message_ = "Hit for " + std::to_string(ev->amount);
            });
        bus_->subscribe<game::mt::service::events::WallBrokenEvent>(
            [this](const std::shared_ptr<game::mt::service::events::WallBrokenEvent>&) {
                last_message_ = "Object destroyed";
            });
        bus_->subscribe<game::mt::service::events::ItemUsedEvent>(
            [this](const std::shared_ptr<game::mt::service::events::ItemUsedEvent>&) {
                last_message_ = "Reloaded";
            });
        bus_->subscribe<game::mt::service::events::MoveEvent>(
            [this](const std::shared_ptr<game::mt::service::events::MoveEvent>& ev) {
                if (!ev || !ev->path || ev->path->empty()) return;
                anim_path_ = *ev->path;
                moving_entity_ = static_cast<game::mt::EntityId>(ev->entity_id);
                animating_ = true;
...
game::mt::controller::InputAction NcursesView::poll_input() {
    int ch = getch();
    switch (ch) {
        case KEY_UP:
        case 'w':
        case 'W':
            return game::mt::controller::InputAction::MOVE_UP;
        case KEY_DOWN:
        case 's':
        case 'S':
            return game::mt::controller::InputAction::MOVE_DOWN;
        case KEY_LEFT:
        case 'a':
        case 'A':
            return game::mt::controller::InputAction::MOVE_LEFT;
        case KEY_RIGHT:
        case 'd':
        case 'D':
            return game::mt::controller::InputAction::MOVE_RIGHT;
        case '\n':
        case KEY_ENTER:
            return game::mt::controller::InputAction::CONFIRM;
        case 27:
        case KEY_BACKSPACE:
            return game::mt::controller::InputAction::CANCEL;
        case '\t':
        case 'm':
        case 'M':
            return game::mt::controller::InputAction::TOGGLE_MODE;
        case 'e':
        case 'E':
            return game::mt::controller::InputAction::END_TURN;
        case 'r':
        case 'R':
            return game::mt::controller::InputAction::RELOAD;
        case 'p':
        case 'P': {
            auto path = enter_path("Save to: ");
            if (path.empty()) return game::mt::controller::InputAction::NONE;
            game::mt::controller::set_io_path(std::move(path));
            return game::mt::controller::InputAction::SAVE_GAME;
        }
        case 'l':
        case 'L': {
            ...
            if (path.empty()) return game::mt::controller::InputAction::NONE;
            game::mt::controller::set_io_path(std::move(path));
            return game::mt::controller::InputAction::LOAD_GAME;
        }
        case 'g':
        case 'G':
            return game::mt::controller::InputAction::PICK_ITEM;
        case 't':
        case 'T':
            return game::mt::controller::InputAction::DROP_ITEM;
        case 'u':
        case 'U':
            return game::mt::controller::InputAction::USE_ITEM;
        case '[':
            return game::mt::controller::InputAction::PREV_CELL_ITEM;
        case ']':
            return game::mt::controller::InputAction::NEXT_CELL_ITEM;
        case ',':
            return game::mt::controller::InputAction::PREV_INV_ITEM;
        case '.':
            return game::mt::controller::InputAction::NEXT_INV_ITEM;
        case 'q':
        case 'Q':
            return game::mt::controller::InputAction::QUIT;
        default:
            return game::mt::controller::InputAction::NONE;
    }
}

void NcursesView::render(const game::mt::service::World& world,
...
static chtype cell_char(const game::mt::repo::cells::ICell* cell) {
    if (!cell) return ' ';
    if (dynamic_cast<const game::mt::repo::cells::Wall*>(cell)) return ACS_BLOCK;
    if (dynamic_cast<const game::mt::repo::cells::Partition*>(cell)) return '+';
    if (dynamic_cast<const game::mt::repo::cells::Glass*>(cell)) return 'g';
    if (dynamic_cast<const game::mt::repo::cells::Stash*>(cell)) return 'S';
    if (dynamic_cast<const game::mt::repo::cells::Floor*>(cell)) return '.';
    return '?';
}

static std::string item_label(const game::mt::entity::items::Item* item) {
    if (!item) return "none";
    if (auto* weapon = dynamic_cast<const game::mt::entity::items::Weapon*>(item)) {
        return "Weapon ammo=" + std::to_string(weapon->get_current_ammo())
            + "/" + std::to_string(weapon->get_max_ammo());
    }
    if (auto* bag = dynamic_cast<const game::mt::entity::items::AmmoBag*>(item)) {
        return "AmmoBag ammo=" + std::to_string(bag->get_current_ammo())
            + "/" + std::to_string(bag->get_max_ammo());
    }
    if (auto* medkit = dynamic_cast<const game::mt::entity::items::Medkit*>(item)) {
...
static double compute_hit_chance(const game::mt::entity::Entity& attacker,
                                 int distance) {
...
void NcursesView::draw_map(const game::mt::service::World& world,
                           const game::mt::controller::Controller& controller) {
    ...
    auto entity_color = [](const game::mt::entity::Entity& ent) {
        return (ent.get_team_id() == 0) ? kFriendlyPair : kEnemyPair;
    };

    for (int y = 0; y < draw_h; ++y) {
        for (int x = 0; x < draw_w; ++x) {
            game::mt::Position pos{cam_x + x, cam_y + y};
            ...
            if (visible) {
                ...
                if (auto* cont = dynamic_cast<const game::mt::repo::cells::IItemContainer*>(cell)) {
                    if (cont->size() > 0) {
                        ch = '*';
                    }
                }
    ...
    if (controller.selected_entity() != game::mt::service::TurnService::kNoEntity) {
        if (auto ent = level->get_entity(controller.selected_entity())) {
            auto* tp = ent->get_component<game::mt::entity::components::TimePointsComponent>();
            auto* mv = ent->get_component<game::mt::entity::components::MoveComponent>();
    ...
    std::shared_ptr<const game::mt::entity::Entity> selected_ent;
    if (controller.selected_entity() != game::mt::service::TurnService::kNoEntity) {
    ...
    if (selected_ent) {
        ...
        if (!cursor_blocked && controller.get_mode() == game::mt::controller::Controller::Mode::MOVE) {
            auto* tp = selected_ent->get_component<game::mt::entity::components::TimePointsComponent>();
            auto* mv = selected_ent->get_component<game::mt::entity::components::MoveComponent>();
            if (!tp || !mv || mv->get_step_cost() <= 0) {
                cursor_blocked = true;
            } else {
...
void NcursesView::draw_hud(const game::mt::service::World& world,
                           const game::mt::controller::Controller& controller) {
    ...
    if (controller.get_mode() == game::mt::controller::Controller::Mode::MOVE) mode = "MOVE";
    if (controller.get_mode() == game::mt::controller::Controller::Mode::ATTACK) mode = "ATTACK";
    ...
    if (controller.selected_entity() != game::mt::service::TurnService::kNoEntity) {
        ...
        if (ent) {
            ...
            if (auto* hp = ent->get_component<game::mt::entity::components::HealthComponent>()) {
                mvprintw(sel_line++, layout.sel_x + 1, "HP: %d/%d",
                         hp->get_current_hp(), hp->get_max_hp());
            }
            if (auto* tp = ent->get_component<game::mt::entity::components::TimePointsComponent>()) {
                mvprintw(sel_line++, layout.sel_x + 1, "TP: %d/%d",
                         tp->get_current_points(), tp->get_max_points());
            }
            if (auto* wp = ent->get_component<game::mt::entity::components::WeaponComponent>()) {
            ...
            if (auto* inv = ent->get_component<game::mt::entity::components::InventoryComponent>()) {
                ...
                if (!items.empty()) {
                    mvprintw(sel_line++, layout.sel_x + 1, "Inv item: %s",
                             item_label(items.front().get()).c_str());
                }
            }
        }
    } else {
    ...
    if (controller.get_mode() == game::mt::controller::Controller::Mode::SELECT) {
    ...
    } else if (controller.get_mode() == game::mt::controller::Controller::Mode::ATTACK
        && controller.selected_entity() != game::mt::service::TurnService::kNoEntity) {
        auto attacker = level->get_entity(controller.selected_entity());
        auto target = level->get_entity_at(controller.get_cursor());
        auto attacker_pos = attacker ? level->get_entity_position(attacker->get_id()) : std::nullopt;
        if (attacker && attacker_pos) {
            ...
            if (target) {
                mvprintw(atk_line++, layout.atk_x + 1, "Attack target: %s",
                         target->get_name().c_str());
                if (auto* hp = target->get_component<game::mt::entity::components::HealthComponent>()) {
                    mvprintw(atk_line++, layout.atk_x + 1, "Target HP: %d/%d",
                             hp->get_current_hp(), hp->get_max_hp());
                }
            } else if (auto* cell_target = level->get_cell(controller.get_cursor())) {
                if (dynamic_cast<const game::mt::repo::cells::IDestructibleCell*>(cell_target)) {
                    mvprintw(atk_line++, layout.atk_x + 1, "Attack target: %s",
            ...
            if (auto* wp = attacker->get_component<game::mt::entity::components::WeaponComponent>()) {
                if (auto weapon = wp->get_weapon()) {
    ...
    if (cell) {
        ...
        if (auto* container = dynamic_cast<game::mt::repo::cells::IItemContainer*>(cell)) {
            ...
            if (!items.empty()) {
                ...
                mvprintw(atk_line++, layout.atk_x + 1, "Cell item [%zu/%zu]: %s",
                         idx + 1, items.size(), item_label(items[idx].get()).c_str());
            }
        }
    } else {
