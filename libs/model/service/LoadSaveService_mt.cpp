...
std::string LoadSaveService::ammo_type_string(game::mt::AmmoType type) {
    switch (type) {
        case game::mt::AmmoType::SHOTGUN: return "SHOTGUN";
        case game::mt::AmmoType::RIFLE: return "RIFLE";
        case game::mt::AmmoType::PISTOL: return "PISTOL";
        case game::mt::AmmoType::SNIPER: return "SNIPER";
    }
    return "PISTOL";
}

bool LoadSaveService::parse_ammo_type(const std::string& token, game::mt::AmmoType& out) {
    if (token == "SHOTGUN") { out = game::mt::AmmoType::SHOTGUN; return true; }
    if (token == "RIFLE") { out = game::mt::AmmoType::RIFLE; return true; }
    if (token == "PISTOL") { out = game::mt::AmmoType::PISTOL; return true; }
    if (token == "SNIPER") { out = game::mt::AmmoType::SNIPER; return true; }
    return false;
}

std::string LoadSaveService::behavior_string(game::mt::entity::components::AIBehavior behavior) {
    using game::mt::entity::components::AIBehavior;
    switch (behavior) {
...
bool LoadSaveService::parse_behavior(const std::string& token, game::mt::entity::components::AIBehavior& out) {
    using game::mt::entity::components::AIBehavior;
...
std::string LoadSaveService::cell_type_string(const game::mt::repo::cells::ICell& cell) {
    if (dynamic_cast<const game::mt::repo::cells::Stash*>(&cell)) return "STASH";
    if (dynamic_cast<const game::mt::repo::cells::Floor*>(&cell)) return "FLOOR";
    if (dynamic_cast<const game::mt::repo::cells::Wall*>(&cell)) return "WALL";
    if (dynamic_cast<const game::mt::repo::cells::Glass*>(&cell)) return "GLASS";
    if (dynamic_cast<const game::mt::repo::cells::Partition*>(&cell)) return "PARTITION";
    return "UNKNOWN";
}

std::unique_ptr<game::mt::repo::cells::ICell> LoadSaveService::make_cell(const std::string& type, bool broken) {
    if (type == "FLOOR") return std::make_unique<game::mt::repo::cells::Floor>();
    if (type == "WALL") return std::make_unique<game::mt::repo::cells::Wall>();
    if (type == "GLASS") {
        auto cell = std::make_unique<game::mt::repo::cells::Glass>();
        if (broken) cell->break_glass();
        return cell;
    }
    if (type == "STASH") return std::make_unique<game::mt::repo::cells::Stash>();
    if (type == "PARTITION") {
        auto cell = std::make_unique<game::mt::repo::cells::Partition>();
...
void LoadSaveService::save_item(std::ostream& out, const game::mt::entity::items::Item& item) {
    if (auto* med = dynamic_cast<const game::mt::entity::items::Medkit*>(&item)) {
    ...
    if (auto* wp = dynamic_cast<const game::mt::entity::items::Weapon*>(&item)) {
        auto dmg = wp->get_dmg();
...
std::unique_ptr<game::mt::entity::items::Item> LoadSaveService::load_item(std::istream& in) {
    std::string kind;
    ...
    if (kind == "MEDKIT") {
        ...
        return std::make_unique<game::mt::entity::items::Medkit>(id, weight, heal, cost);
    }
    if (kind == "AMMOBAG") {
        game::mt::ItemId id = 0;
        int weight = 0;
        ...
        game::mt::AmmoType type{};
        if (!parse_ammo_type(ammo, type)) return nullptr;
        return std::make_unique<game::mt::entity::items::AmmoBag>(id, weight, cur, maxv, type);
    }
    if (kind == "WEAPON") {
        game::mt::ItemId id = 0;
        int weight = 0;
        ...
        return std::make_unique<game::mt::entity::items::Weapon>(
            id, weight, game::mt::Damage{min_dmg, max_dmg}, range, attack_cost, reload_cost, type, cur, maxv);
    }
    return nullptr;
}
...
bool LoadSaveService::save(const World& world, const std::string& path) const {
    ...
    for (int x = 0; x < static_cast<int>(level->get_width()); ++x) {
        for (int y = 0; y < static_cast<int>(level->get_height()); ++y) {
            ...
            bool broken = false;
            if (auto* glass = dynamic_cast<const game::mt::repo::cells::Glass*>(cell)) broken = glass->is_broken();
            if (auto* part = dynamic_cast<const game::mt::repo::cells::Partition*>(cell)) broken = part->is_broken();

            std::vector<std::shared_ptr<const game::mt::entity::items::Item>> items;
...
std::unique_ptr<World> LoadSaveService::load(const std::string& path) const {
    ...
    auto level = std::make_unique<game::mt::repo::Level>(level_id, level_name);
    ...
    for (int x = 0; x < static_cast<int>(width); ++x) {
        for (int y = 0; y < static_cast<int>(height); ++y) {
            ...
            if (items_count > 0) {
                auto* cont = dynamic_cast<game::mt::repo::cells::IItemContainer*>(level->get_cell({cx, cy}));
    ...
    for (size_t i = 0; i < entity_count; ++i) {
        std::string ent_tag;
        game::mt::EntityId id = 0;
        std::string name;
        game::mt::TeamId team = 0;
        int x = 0;
        int y = 0;
        if (!(in >> ent_tag >> id >> std::quoted(name) >> team >> x >> y)) return nullptr;
        if (ent_tag != "ENTITY") return nullptr;

        auto ent = std::make_unique<game::mt::entity::Entity>(id, name, team);
            ...
            if (comp_type == "HEALTH") {
                ...
                if (maybe_none == "NONE") {
                    ent->add_component<entity::components::WeaponComponent,
                        entity::components::DefaultWeaponComp>(std::unique_ptr<game::mt::entity::items::Weapon>{});
                } else {
                    game::mt::ItemId wid = 0;
                    int weight = 0;
                    int min_dmg = 1;
                    int max_dmg = 1;
                    int range = 0;
                    ...
                    int maxv = 0;
                    try {
                        wid = static_cast<game::mt::ItemId>(std::stoull(maybe_none));
                    ...
                    game::mt::AmmoType type{};
                    if (!parse_ammo_type(ammo, type)) return nullptr;
                    auto weapon = std::make_unique<game::mt::entity::items::Weapon>(
                        wid, weight, game::mt::Damage{min_dmg, max_dmg}, range, attack_cost, reload_cost, type, cur, maxv);
                    ent->add_component<entity::components::WeaponComponent,
