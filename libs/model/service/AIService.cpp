...
AIService::Choice AIService::choose_best_action(World& w, EntityId id) {
    ...
    if (inv_exists) {
        if (auto* cell = level->get_cell(*pos)) {
            if (auto* cont = dynamic_cast<game::repo::cells::IItemContainer*>(cell)) {
    ...
    if (inv_has_items) {
        if (auto* cell = level->get_cell(*pos)) {
            if (dynamic_cast<game::repo::cells::Stash*>(cell)) {
    ...
    if (enemy) {
        ...
        if (enemy->line_of_fire && game::service::CombatService::can_shoot(*ent)) {
            Choice c;
...
std::vector<Position> AIService::find_stashes(const game::repo::Level& level) {
    ...
    for (int x = 0; x < static_cast<int>(level.get_width()); ++x) {
        for (int y = 0; y < static_cast<int>(level.get_height()); ++y) {
            ...
            if (cell && dynamic_cast<const game::repo::cells::Stash*>(cell)) {
                res.push_back({x, y});
            }
        }
    }
...
std::vector<Position> AIService::find_item_cells(const game::repo::Level& level) {
    std::vector<Position> res;
    for (int x = 0; x < static_cast<int>(level.get_width()); ++x) {
        for (int y = 0; y < static_cast<int>(level.get_height()); ++y) {
            auto* cell = level.get_cell({x, y});
            auto* container = dynamic_cast<const game::repo::cells::IItemContainer*>(cell);
...
bool AIService::pickup_one_here(game::repo::Level& level, game::EntityId id) {
...
bool AIService::deposit_all_here(game::repo::Level& level, game::EntityId id) {
    auto* ent = level.get_entity(id);
    if (!ent) return false;
    ...
    auto* cell = level.get_cell(*pos);
    auto* stash = dynamic_cast<game::repo::cells::Stash*>(cell);
    if (!stash) return false;

    return drop_all_to_cell(*stash, *inv);
}

bool AIService::drop_all_to_cell(game::repo::cells::IItemContainer& cell,
