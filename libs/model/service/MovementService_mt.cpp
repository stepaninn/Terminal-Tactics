...
bool MovementService::move(game::mt::repo::Level& level, game::mt::EntityId id, game::mt::Position to) const {
    auto lock = EntityLockPool::instance().lock_entity(id);
    auto entity = level.get_entity(id);
    if (!entity) return false;

    auto* mv = entity->get_component<game::mt::entity::components::MoveComponent>();
    auto* tp = entity->get_component<game::mt::entity::components::TimePointsComponent>();
...
std::vector<game::mt::Position> MovementService::find_path(const game::mt::repo::Level& level,
                                                       game::mt::EntityId id,
                                                       game::mt::Position to) {
    ...
    std::vector<std::vector<game::mt::Position>> parent(
        rows, std::vector<game::mt::Position>(cols, {-1, -1}));

    std::queue<game::mt::Position> q;
    ...
    // воссатновление пути
    std::vector<game::mt::Position> path;
...
std::vector<game::mt::Position> MovementService::find_path_without_target(const game::mt::repo::Level& level,
                                                                      game::mt::EntityId id,
                                                                      game::mt::Position to) {
    if (!level.check_entity(id)) return {};
    ...
    occupied[static_cast<size_t>(to.x)][static_cast<size_t>(to.y)] = false;
