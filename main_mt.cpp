...
int main() {
    ...
    auto level = std::make_unique<game::mt::repo::Level>(1, "Demo");
    ...
    auto set_glass = [&](int x, int y) {
        level->set_cell(game::mt::Position{x, y}, std::make_unique<game::mt::repo::cells::Glass>());
    };
    auto set_partition = [&](int x, int y) {
        level->set_cell(game::mt::Position{x, y}, std::make_unique<game::mt::repo::cells::Partition>());
    };
    ...
    level->set_cell(game::mt::Position{3, 3}, std::make_unique<game::mt::repo::cells::Stash>());
    level->set_cell(game::mt::Position{34, 18}, std::make_unique<game::mt::repo::cells::Stash>());

    auto add_operative = [&](game::mt::EntityId id, std::string name, game::mt::Position pos, game::mt::AmmoType ammo) {
        auto weapon = std::make_unique<game::mt::entity::items::Weapon>(
            id * 10 + 1, 2, game::mt::Damage{1, 4}, 6, 1, 2, ammo, 6, 8);
    ...
    auto add_enemy = [&](game::mt::EntityId id,
                         std::string name,
                         game::mt::Position pos,
                         game::mt::entity::components::AIBehavior behavior,
                         game::mt::AmmoType ammo) {
        auto weapon = std::make_unique<game::mt::entity::items::Weapon>(
            id * 10 + 2, 2, game::mt::Damage{1, 3}, 5, 1, 2, ammo, 4, 6);
    ...
    add_operative(1, "Alpha", {3, 10}, game::mt::AmmoType::PISTOL);
    add_operative(2, "Bravo", {5, 11}, game::mt::AmmoType::RIFLE);
    add_operative(3, "Charlie", {4, 13}, game::mt::AmmoType::SHOTGUN);

    add_enemy(10, "Raider", {32, 4}, game::mt::entity::components::AIBehavior::WILD, game::mt::AmmoType::SHOTGUN);
    add_enemy(11, "Sniper", {29, 17}, game::mt::entity::components::AIBehavior::INTELLIGENT, game::mt::AmmoType::SNIPER);
    add_enemy(12, "Scavenger", {20, 18}, game::mt::entity::components::AIBehavior::FORAGER, game::mt::AmmoType::PISTOL);

    if (auto ent = level->get_entity(1)) {
        if (auto* inv = ent->get_component<game::mt::entity::components::InventoryComponent>()) {
            inv->add(std::make_shared<game::mt::entity::items::Medkit>(100, 2, 5, 2));
            inv->add(std::make_shared<game::mt::entity::items::AmmoBag>(101, 1, 6, 12, game::mt::AmmoType::PISTOL));
        }
    }
    if (auto ent = level->get_entity(2)) {
        if (auto* inv = ent->get_component<game::mt::entity::components::InventoryComponent>()) {
            inv->add(std::make_shared<game::mt::entity::items::AmmoBag>(102, 1, 6, 12, game::mt::AmmoType::RIFLE));
        }
    }
    if (auto ent = level->get_entity(3)) {
        if (auto* inv = ent->get_component<game::mt::entity::components::InventoryComponent>()) {
            inv->add(std::make_shared<game::mt::entity::items::Medkit>(103, 2, 4, 2));
        }
    }

    level->add(game::mt::Position{3, 3},
               std::make_shared<game::mt::entity::items::Medkit>(200, 2, 4, 2));
    level->add(game::mt::Position{34, 18},
               std::make_shared<game::mt::entity::items::AmmoBag>(201, 1, 8, 12, game::mt::AmmoType::PISTOL));
    level->add(game::mt::Position{18, 10},
               std::make_shared<game::mt::entity::items::AmmoBag>(202, 1, 4, 8, game::mt::AmmoType::SHOTGUN));
    ...
    game::mt::view::NcursesView view(bus);
