...
public:
    ...
    [[nodiscard]] Entity* get_entity(id_t id) { return entities_[id].get(); }
    ...
    std::unique_ptr<Entity> remove_entity(id_t id);
...
private:
    ...
    std::unordered_map<id_t, std::unique_ptr<Entity>> entities_;
    std::unordered_map<id_t, Position> entity_positions_;

    id_t next_entity_id_{1};
};
