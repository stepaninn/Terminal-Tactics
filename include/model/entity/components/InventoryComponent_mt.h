#ifndef MYGAMEPROJECT_INVENTORYCOMPONENT_MT_H
#define MYGAMEPROJECT_INVENTORYCOMPONENT_MT_H

#include "IComponent_mt.h"
#include "types_mt.h"
#include "../entities/items/Item_mt.h"
#include <mutex>
#include <memory>
...
public:
     ...
     */
    virtual void add(std::shared_ptr<game::mt::entity::items::Item> item) = 0;
    /**
     ...
     */
    virtual std::shared_ptr<game::mt::entity::items::Item> remove_by_id(game::mt::id_t id) = 0;
     ...
     */
    [[nodiscard]] virtual std::shared_ptr<const game::mt::entity::items::Item> get_item(
        game::mt::id_t id) const noexcept = 0;

    /**
     * @brief Метод получения предмета в инвентаре
     * @param id ID желаемого предмета
     * @return Указатель на предмет
     */
    [[nodiscard]] virtual std::shared_ptr<game::mt::entity::items::Item> get_item(
        game::mt::id_t id) noexcept = 0;
     ...
     */
    [[nodiscard]] virtual bool can_add(const game::mt::entity::items::Item& item) const noexcept = 0;
...
public:
    ...
    void add(std::shared_ptr<game::mt::entity::items::Item> item) override;
    std::shared_ptr<game::mt::entity::items::Item> remove_by_id(game::mt::id_t id) override;
    ...
    [[nodiscard]] std::shared_ptr<game::mt::entity::items::Item> get_item(game::mt::id_t id) noexcept override;
    [[nodiscard]] std::shared_ptr<const game::mt::entity::items::Item> get_item(
        game::mt::id_t id) const noexcept override;

    [[nodiscard]] int get_weight() const noexcept override {
        std::lock_guard<std::mutex> lock(mutex_);
        return weight_;
    }
    ...
    [[nodiscard]] bool can_add(const game::mt::entity::items::Item& item) const noexcept override;
...
protected:
    ...
    int weight_ = 0;
    int max_weight_ = 0;
    tbb::concurrent_hash_map<game::mt::id_t, std::shared_ptr<game::mt::entity::items::Item>> items_;
};

}

#endif //MYGAMEPROJECT_INVENTORYCOMPONENT_MT_H
