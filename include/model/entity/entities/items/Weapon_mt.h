#ifndef MYGAMEPROJECT_WEAPON_MT_H
#define MYGAMEPROJECT_WEAPON_MT_H
...
public:
    Weapon() = default;
    Weapon(game::mt::ItemId id, int weight, game::mt::Damage dmg, int range, int attack_cost, int reload_cost,
           game::mt::AmmoType type, int cur_ammo, int max_ammo) :
            Item(id, weight),
            dmg_(dmg),
     ...
     */
    [[nodiscard]] game::mt::Damage get_dmg() const noexcept {
     ...
     * @param d новый урон
     */
    void set_dmg(game::mt::Damage d) noexcept {
        std::lock_guard<std::mutex> lock(mutex_);
        dmg_ = d;
    }
     ...
     */
    [[nodiscard]] int get_attack_cost() const noexcept {
        std::lock_guard<std::mutex> lock(mutex_);
        return attack_cost_;
    }
    /**
     ...
     */
    [[nodiscard]] int get_reload_cost() const noexcept {
        std::lock_guard<std::mutex> lock(mutex_);
        return reload_cost_;
    }
...
#endif //MYGAMEPROJECT_WEAPON_MT_H
